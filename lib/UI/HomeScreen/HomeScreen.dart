...
class _HomescreenState extends State<Homescreen> {
  ...
  void fetchNewMessages() {
    FirebaseFirestore.instance.collection('chats').where('userId', isEqualTo: FirebaseAuth.instance.currentUser!.uid).snapshots().listen((querySnapshot) {
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      ...
      // 2. Fetch products WITH A LIMIT for pagination
      // We remove the .get() from the whole collection and add a limit
      QuerySnapshot productSnapshot = await FirebaseFirestore.instance
          .collection('products')
          .limit(12)
          .get();

      List<Product> fetchedProducts = productSnapshot.docs
          .map((doc) => Product.fromDocument(doc))
          .where((product) => validSellerIds.contains(product.sellerId))
          .toList();

      // 3. Sort & Shuffle (Only for this batch)
      fetchedProducts.sort((a, b) {
        double ratingA = double.tryParse(a.avgRate.toString()) ?? 0.0;
        double ratingB = double.tryParse(b.avgRate.toString()) ?? 0.0;
      ...
      setState(() {
        ...
        _isLoading = false;
        _applyCategoryFilter(); // Call this AFTER setting _lastDocument
      });
    } catch (e) {
  ...
  void _applyCategoryFilter() {
    setState(() {
      List<Product> baseList = _products.where((p) => p.category != "Rental").toList();
      ...
      // REMOVED: .take(10) - This was preventing more items from showing
    });
  }
  Future<void> _loadMoreProducts() async {
    ...
    try {
      // 1. Re-fetch or use the valid Seller IDs
      // (In a real app, you might want to store validSellerIds as a class variable
      // so you don't fetch them every time you scroll)
      QuerySnapshot sellerSnapshot = await FirebaseFirestore.instance
          .collection('saller')
          .where('isAdminApproved', isEqualTo: true)
          .where('isSellerRestricted', isEqualTo: false)
      ...
      // 2. Query the next batch of products
      Query query = FirebaseFirestore.instance.collection('products')
          .startAfterDocument(_lastDocument!)
          .limit(20); // Fetch a slightly larger batch to account for filtered-out sellers

      QuerySnapshot productSnapshot = await query.get();

      if (productSnapshot.docs.isNotEmpty) {
        final newFetchedProducts = productSnapshot.docs
            .map((doc) => Product.fromDocument(doc))
            .where((product) => validSellerIds.contains(product.sellerId))
            .toList();

        setState(() {
          _products.addAll(newFetchedProducts);
          _lastDocument = productSnapshot.docs.last;
          _applyCategoryFilter(); // Ensure the display list updates
        });
      } else {
        _lastDocument = null; // No more data to fetch
      }
    } catch (e) {
  ...
  Widget _buildSliverAppBar() {
    ...
    return SliverAppBar(
      ...
      actions: [
        _buildAppIcon(Icons.notifications_none_outlined, () => Navigator.push(context, MaterialPageRoute(builder: (_) => UniversalNotificationScreen(currentUserId: _auth.currentUser!.uid, userRole: 'customer',)))),
        _buildAppIcon(Icons.favorite_border, () => Navigator.push(context, MaterialPageRoute(builder: (_) => WishListScreen(userId: _auth.currentUser!.uid,)))),
        _buildChatIcon(),
...
// 3. Gradient Helper (Required for the banner)
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildActionItem(Icons.history, "Orders", Colors.purple, () =>Navigator.push(context, MaterialPageRoute(builder: (_) => const Ordersscreen()))),
  ...
  void _navigateToFullView(Product product) {
    Navigator.push(context, MaterialPageRoute(builder: (context) => customerProductfullview(
